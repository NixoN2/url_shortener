...
module.exports = createCoreController('api::shortener.shortener', ({strapi}) =>  ({
  async find(ctx) {
    let { query } = ctx;
    ...
    if (user) {
      ...
      if (entity.results.length !== 0){
        ...
        await strapi.service('api::shortener.shortener').update(id, { data: { visit } });
      }
    }
  ...
  async create(ctx) {
    ...
    let entity;
    data.user = user.id;
    entity = await strapi.service('api::shortener.shortener').create({ data });
    const sanitizedEntity = await this.sanitizeOutput(entity, ctx);
    return this.transformResponse(sanitizedEntity);
  },
  async delete(ctx) {
